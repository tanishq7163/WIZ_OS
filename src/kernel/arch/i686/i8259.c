...
enum {
    ...
    PIC_ICW4_BUFFER_MASTER      = 0x1,
    PIC_ICW4_BUFFER_SLAVE       = 0x1,
    PIC_ICW4_SFNM               = 0x1,
} PIC_ICW4;
...
void i8259_Configure(uint8_t offsetPic1, uint8_t offsetPic2, bool autoEoi)
{
    ...
    i686_outb(PIC1_DATA_PORT, 0x4);         // tells PIC1 that it has a slave at IRQ2 (0000 0100)
    i686_iowait();
    i686_outb(PIC2_DATA_PORT, 0x2);         // tells PIC1 that its cascade identity (0000 0010)
    i686_iowait();
